...
use crate::overworld::{
    asset::character::{CharacterAsset, CharacterVisual},
    input::InputSystems,
    tile::Tile,
};
use bevy::prelude::*;
use engine::{
    ...
    asset::{AssetResolver, AssetsExt, HasResolver, overworld::CHARACTER_LAYER},
    overworld::tile::{Grid, GridSize, Neighbor, Passability, TILE_SIZE},
};

const PLAYER_SPEED: f32 = TILE_SIZE.x as f32 / 14.0;
...
struct Character(Handle<CharacterAsset>);
...
fn start_tile_transition(
    ...
    mut character: Query<(Entity, &Transform, &Orientation)>,
...
fn move_character(
    mut character: Query<(Entity, &Orientation, &mut Transform, &mut TileTransition)>,
    grid_size: Single<&GridSize>,
    mut commands: Commands,
) {
    for (entity, orientation, mut transform, mut tt) in &mut character {
        ...
        let distance_to_origin =
            (new_translation.truncate() - grid_size.grid_to_world(tt.origin.as_vec2())).length();

        match &tt.state {
            TileTransitionState::LeavingTile => {
                if distance_to_origin >= TILE_SIZE.x as f32 / 2.0 {
                    tt.state = TileTransitionState::EnteringTile;
                }
            }
            TileTransitionState::EnteringTile => {
                if distance_to_origin >= TILE_SIZE.x as f32 {
                    commands.entity(entity).remove::<TileTransition>();
                    new_translation = grid_size
                        .grid_to_world(tt.target.as_vec2())
...
fn update_visuals(
    ...
    for (orientation, state, character, children) in &mut character {
        ...
        for child in children {
            ...
            let key =
                crate::overworld::asset::character::CharacterState::from((*state, *orientation));
